...
namespace SoALLoadDetector
{
	...
	internal class FeatureDetector
	{
		...
		private static bool checkBlackLevelTransition(int[] newVector, int[,] comparison_vectors, List<int> max_per_patch, List<int> min_per_patch, float max_transition_threshold, out float average_max_transition, out int matchingBins, float percentageOfBinsCorrectOverride = -1.0f, bool debugOutput = true, int black_level_config = 10)
		{
			//int[,] comparison_vectors = listOfFeatureVectorsEng;
			int size = newVector.Length;
		...
		public static bool compareFeatureVectorTransition(int[] newVector, int[,] comparison_vectors, List<int> max_per_patch, List<int> min_per_patch, float max_transition_threshold, out float average_max_transition, out int matchingBins, float percentageOfBinsCorrectOverride = -1.0f, bool debugOutput = true, int black_level_config = 10)
		{

			if (checkBlackLevelTransition(newVector, comparison_vectors, max_per_patch, min_per_patch, max_transition_threshold, out average_max_transition, out matchingBins, percentageOfBinsCorrectOverride, debugOutput, black_level_config))
				return true;
